...
correct_op = tf.equal(tf.argmax(ema_predictions_op, axis = -1), tf.argmax(label_var, axis = -1))
accuracy_op = tf.reduce_mean(tf.cast(correct_op, tf.float32)) * 100

learning_rate_var = tf.placeholder(tf.float32)
with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
    train_op = tf.train.AdamOptimizer(learning_rate_var).minimize(loss_op, var_list = train_vars)
...
train_writer = tf.summary.FileWriter('./logs/train_100%_with_MixUp_and_EMA')
...
saver.save(sess, './model/MixUp_100%_with_EMA.ckpt')
